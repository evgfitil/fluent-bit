...
struct opentelemetry_context *flb_opentelemetry_context_create(struct flb_output_instance *ins, struct flb_config *config)
{
    ...
    if (ctx->metadata_token_url) {
        /*
         * metadata_token_refresh must exceed FLB_OAUTH2_DEFAULT_SKEW_SECS (60s).
         * If refresh <= 60, flb_oauth2_get_access_token() sees a freshly-fetched
         * token as already-expired (expires_at - 60 <= now), triggers the oauth2
         * POST refresh, which fails (no client_id/secret), and returns FLB_RETRY
         * on every flush -- no data is ever delivered.
         */
...
void flb_opentelemetry_context_destroy(struct opentelemetry_context *ctx)
{
    ...
    /*
     * ctx->oauth2_config fields are views into the config map's kv list (owned
     * by the output instance) and must not be freed here; the instance lifecycle
     * handles that.
     */
